...
parser.add_argument('--expe', dest='expe', nargs=1, default='V',
			  help='Select experiment id (C vs. V)')

parser.add_argument('--colstim', dest='colstim', nargs=1, default='1',
                   help='Select which column number of the file contains the stimulus names (default: first column).')

parser.add_argument('--liste', dest='liste', nargs=1, default='1',
                   help='Select list number (default: 1).')
...
# Tests
trainstimuli = pyllar.read_data_2D('vocoderConfusion-training-'+expenum+'.conf',';')
print(trainstimuli)
for i in range(0, len(trainstimuli)):
        if not os.path.isfile("sons/V/"+trainstimuli[i][0]):
                print(trainstimuli[i][0], "does not exist")
teststimuli = pyllar.read_data_2D('vocoderConfusion-expe-'+expenum+'-liste'+listenum+'.conf',';')
for i in range(0, len(teststimuli)):
        if not os.path.isfile("sons/V/"+teststimuli[i][colstim]):
                print(teststimuli[i][colstim], "does not exist")
...
if expenum=='C':
    ...
    matrice = ['','...i','...é','...a','...an','','','','...u','...eu','','...on','','','','...ou','...o','','...in','','']
...
def main():
    ...
    expe('vocoderConfusion-expe-'+expenum+'-liste'+listenum+'.conf','resultats-vocoderConfusion-expe-'+expenum+'.res')
...
def expe(conf,res):
    ...
    if expenum=='C':
        ...
        pyllar.displaymatrice_3_7('','...i','...é','...a','...an','','', '','...u','...eu','','...on','','','','...ou','...o','','...in','','')
    ...
    for i in alea:
        ...
        answers = matrice
        pyllar.pause(200)
        soundpath="sons/"+expenum+"/"+stimuli[i][colstim]
        print i,stimuli[i][colstim]
        ...
        resultsdata = (sujet+";liste"+str(listenum)+";"+str(expeStart)+";"+str(savedata)+str(i+1)+";"+str(x)+";"+str(y)+";"+str(button)+";"+reponse+"\n")
